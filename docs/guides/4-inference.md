...
You have successfully trained or fine-tuned a TTS model and selected a promising checkpoint. Now you can use that custom model to convert new text into speech audio, a process called **inference** or **synthesis**.

If a training or audio term feels unfamiliar while you read, use the [glossary](../glossary.md#glossary-of-technical-terms). This page only pauses to explain terms when they affect a practical inference decision.
...
### Locate the Inference Script and Best Checkpoint
...
- **Best Checkpoint:** Identify the path to the model [checkpoint](../glossary.md#glossary-checkpoint) (`.pth`, `.pt`, `.ckpt`) you want to use. This is typically the one saved as `best_model.pth` (based on [validation loss](../glossary.md#glossary-validation-loss)) or another checkpoint you selected by listening to validation samples during training. It will be located within your training output directory (e.g., `../checkpoints/my_yoruba_voice_run1/best_model.pth`).
- **Configuration File:** You will almost always need the configuration file (`.yaml`, `.json`) that was used *during the training* of the checkpoint you are using. The inference script needs this to know the model architecture, audio parameters such as [sampling rate](../glossary.md#glossary-sampling-rate), and other settings. Often, a copy of the config is saved alongside the checkpoints.
- **First-run rule:** Treat checkpoint + config as a matched pair. If you are not sure they came from the same run, assume they do **not** match until you verify it.

### Basic Single Sentence Inference
...
- **Key Arguments:**
  ...
  - `--speaker_id` or `--spk`: **Required** if you trained a multi-speaker model. Provide the exact speaker ID used in your [manifest files](../glossary.md#glossary-manifest-file) for the desired voice. For single-speaker models, this might be optional or ignored.
  - `--device`: Often optional, defaults to `cuda` if available, otherwise `cpu`. If you are unsure what `cuda` means here, see the [CUDA glossary entry](../glossary.md#glossary-cuda). Inference is much faster on GPU.
...
#### First Inference Smoke Test

For your first test, do not start with a long paragraph or a giant batch file. Use one short sentence such as:

```text
Hello, this is a short test sentence.
```

If that fails, fix the basic pipeline first. Batch inference will not fix a broken config, wrong checkpoint, or bad speaker ID.

### Batch Inference (Synthesizing from a File)
...
### Advanced Inference Controls (Framework Dependent)
...
- **CUDA Out-of-Memory (OOM):** Even if training worked, very long sentences during inference can still consume more memory than expected. Try shorter sentences first, then look for framework options for segmented synthesis or CPU fallback.
- **Model/Config Mismatch:** Using a checkpoint with the wrong configuration file is one of the most common inference failures. The model may fail to load, or it may load and produce garbage output. Treat this as a first-check item, not a last resort.
- **Incorrect Speaker ID:** Providing a non-existent speaker ID for multi-speaker models will usually cause an error or produce the wrong voice.
- **Output Path Problems:** Some inference scripts do not create missing output folders automatically. If nothing is saved, verify the exact output path before debugging the model itself.
- **Quality Issues (Noise, Instability):** If output quality is poor, revisit Guide 1 (Data Preparation) and Guide 3 (Model Training). Inference rarely fixes a weak dataset, unstable training run, or badly chosen checkpoint.

#### Quick Inference Triage

When inference fails, check these in order:

1. Did the script actually load the checkpoint and config without warnings?
2. Does the output path exist and is it writable?
3. For multi-speaker models, did you pass the exact speaker ID from the manifests?
4. Did you test one short sentence before trying long text or batches?
5. If the file was created but sounds wrong, did you verify checkpoint/config pairing before changing text or advanced settings?

---

## Optional: Model Evaluation and Deployment

This section is intentionally optional. If you are a beginner, do not block on MOS studies, ASR-based metrics, or deployment architecture before you can reliably generate a few good local samples.

For most personal and first-project use cases, local listening tests are enough to decide whether a checkpoint is worth keeping. Treat the metrics below as tools for comparison and debugging, not as a requirement before you can use your model.

### Evaluating TTS Model Quality

While subjective listening tests are still the gold standard for TTS evaluation, objective metrics can help you compare runs more systematically:
...
**Practical note:** This example is for experimentation, not a production-ready evaluation pipeline. Start by listening to a consistent small test set, then add objective metrics only if you need clearer comparisons between checkpoints or model versions.
...
**Scope note:** deployment is a separate engineering problem from training a usable model. If you are still fixing pronunciation, instability, or speaker mismatch, keep working locally first.

**Practical rule:** do not start with Kubernetes, autoscaling, or serverless infrastructure unless you already have a stable local inference command and a repeatable way to load the model. Local reliability comes first.
...
For many maintainers, a small local wrapper script or a lightweight Gradio demo is enough for a long time. You do not need a production stack just to make the model usable on your own machine or to share a demo with a few testers.
...
- [ ] The checkpoint file and config file come from the same training run.
- [ ] You tested one short sentence before trying a large batch job.
- [ ] Your output path or output directory exists and is writable.
- [ ] You supplied the correct speaker ID for multi-speaker models, if required.
- [ ] If output sounds wrong, you verified sampling rate, config match, and checkpoint selection before changing inference text.
